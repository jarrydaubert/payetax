...
| `P2-29` | Make the `www` to apex redirect permanent | Inspect Vercel/domain routing so `https://www.payetax.co.uk/` redirects to `https://payetax.co.uk/` with a permanent status, then verify the final response chain for the homepage plus representative marketing, blog, and calculator routes. | `https://www.payetax.co.uk/`, one representative blog URL, and one representative calculator URL each return a permanent redirect (`308` preferred) to the apex host with no extra redirect hop after host normalization, the canonical host policy is documented in the relevant SEO/ops config or runbook, and command output proving the final redirect chain is attached to the implementation evidence. |
