...
| `P2-16` | Expand Linear SDK automation for Kanban discipline | Add `scripts/linear.js` commands for backlog-sync checks, DoR field validation on status transitions, and done-vs-backlog burn-down hygiene reporting. | Team can run one command to detect backlog/board drift and missing readiness/done criteria before release planning. |
