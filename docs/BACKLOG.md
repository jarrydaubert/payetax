...
- Consolidate income-tax band mechanics: rUK and Scottish slicing each have one owner; the display interpreters remain.
- One tax-code grammar shared by engine, decoder, validation schema and store edit-gate (currently 4).
- Remove shadow helpers that tests exercise but production never calls (`taxRateDescriptions`).
