...
async function trackApi(method, path, body) {
  if (!hasTrackAuth) {
    return {
      error: 'Track API requires CUSTOMERIO_SITE_ID and CUSTOMERIO_API_KEY environment variables',
    };
  }
  if (args['dry-run']) {
    return {
  ...
  switch (cmd) {
    case 'customers':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown customers subcommand. Use: identify, get, delete, track-event',
          };
      }
      break;
