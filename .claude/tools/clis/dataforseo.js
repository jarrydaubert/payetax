...
if (!(LOGIN && PASSWORD)) {
  console.error(
    JSON.stringify({
      error: 'DATAFORSEO_LOGIN and DATAFORSEO_PASSWORD environment variables required',
    }),
  );
  process.exit(1);
}
  ...
  switch (cmd) {
    ...
    case 'keywords':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown keywords subcommand. Use: volume, for-site, for-keywords, trends',
          };
      }
      break;
    ...
    case 'backlinks':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown backlinks subcommand. Use: summary, list, refdomains, anchors, index',
          };
      }
      break;
      ...
      switch (sub) {
        ...
        case 'domain-intersection': {
          ...
          if (!targets || targets.length < 2) {
            result = { error: '--targets required (comma-separated, at least 2 domains)' };
            break;
          }
