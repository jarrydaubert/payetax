...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'KEYWORDS_EVERYWHERE_API_KEY environment variable required' }),
  );
  process.exit(1);
}
  ...
  switch (cmd) {
    case 'keywords':
      switch (sub) {
        case 'data': {
          ...
          if (!kw) {
            result = { error: '--kw required (comma-separated keywords, max 100)' };
            break;
          }
    ...
    case 'domain':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown domain subcommand. Use: keywords, traffic, backlinks, unique-backlinks',
          };
      }
      break;
    ...
    case 'url':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown url subcommand. Use: keywords, traffic, backlinks, unique-backlinks',
          };
      }
      break;
