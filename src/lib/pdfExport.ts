...
export interface ExportData {
  results: any; // TaxCalculationResults type
  salary: number;
  taxYear: string;
  taxCode: string;
  region: string;
...
export async function generatePDF(data: ExportData): Promise<void> {
  try {
    ...
    // Table rows
    const tableData = [
      ['Gross Salary', results.grossSalary.annually, results.grossSalary.monthly, results.grossSalary.weekly],
      ['Income Tax', -results.incomeTax.annually, -results.incomeTax.monthly, -results.incomeTax.weekly],
      ['National Insurance', -results.nationalInsurance.annually, -results.nationalInsurance.monthly, -results.nationalInsurance.weekly],
    ];
    
    if (results.pensionContribution.annually > 0) {
      tableData.push(['Pension Contribution', -results.pensionContribution.annually, -results.pensionContribution.monthly, -results.pensionContribution.weekly]);
    }
    
    if (results.studentLoan.annually > 0) {
      tableData.push(['Student Loan Repayment', -results.studentLoan.annually, -results.studentLoan.monthly, -results.studentLoan.weekly]);
    }
    
    pdf.setFont('helvetica', 'normal');
    tableData.forEach(([description, annual, monthly, weekly]) => {
    ...
    const effectiveRate = ((results.incomeTax.annually + results.nationalInsurance.annually) / results.grossSalary.annually * 100).toFixed(1);
    const incomeTaxRate = (results.incomeTax.annually / results.grossSalary.annually * 100).toFixed(1);
    const niRate = (results.nationalInsurance.annually / results.grossSalary.annually * 100).toFixed(1);
    ...
    pdf.setFontSize(8);
    pdf.text('Generated by ToolHubX UK Tax Calculator - https://toolhubx.uk', 20, 190);
