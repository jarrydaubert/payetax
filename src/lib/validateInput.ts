...
export function validateCalculatorInput(input: TaxCalculationInput): ValidationResult {
  ...
  if (sanitized.age !== undefined) {
    if (typeof sanitized.age !== 'number' || Number.isNaN(sanitized.age)) {
  ...
  if (sanitized.pensionContribution < 0) {
    ...
    sanitized.pensionContributionType === 'percentage' &&
    sanitized.pensionContribution > 100
  ) {
...
export function sanitizeSalary(salary: number | string | unknown): number {
  ...
  if (typeof salary === 'number') {
    return Number.isNaN(salary) ? 0 : salary;
  }
