...
function isIndexNowRequestBody(value: unknown): value is IndexNowRequestBody {
  return (
    isRecord(value) &&
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (!isIndexNowRequestBody(parsed)) {
      ...
      const invalidUrls = urls.filter((url) => !isValidPayetaxUrl(url));
      ...
      try {
        ...
        console.error(
          `[IndexNow:${requestId}] Fetch failed: ${isTimeout ? 'timeout' : fetchError}`,
        );
        return NextResponse.json(
