    ...
    <div className={cn('relative', className)}>
      {label && (
        <label
          id={labelId}
          htmlFor={selectId}
          className={cn('block text-sm font-medium text-foreground mb-1', hideLabel && 'sr-only')}
        >
          {label}
        </label>
      )}
        ...
        {({ open }) => (
          <div className="relative">
            <ListboxButton
              ...
              className={cn(
                'relative w-full py-2 pl-3 pr-10 text-left rounded-lg glass-input',
                'backdrop-blur-glass-sm border-glass shadow-glass-sm',
                'focus:outline-none focus:ring-1 focus:ring-primary focus:glow-sm',
                'cursor-default transition-all duration-200',
                'bg-glass-deep text-foreground',
                'disabled:cursor-not-allowed disabled:opacity-50',
                open && 'ring-1 ring-primary glow-sm'
              )}
            >
            ...
            >
              <ListboxOptions
                id={listboxId}
                className={cn(
                  'absolute z-10 w-full mt-1 overflow-auto bg-glass',
                  'backdrop-blur-glass-sm max-h-60 rounded-lg shadow-glass',
                  ...
                  <ListboxOption
                    ...
                    className={({ active, selected }) =>
                      cn(
                        ...
                        'focus:outline-none focus:bg-glass-deep focus:text-primary'
                      )
                    }
                  >
                    {({ selected }) => (
